...
function of_load_only() {
	...
	wp_enqueue_media();//this is for media upload
}
...
<script type="text/javascript" language="javascript">
		...
		$(document).ready( function() {
			...
			$( '.image_upload_button_media' ).on( 'click', function( event ) {
				event.preventDefault();
		
				var uploader_id = this.id;
				...
				file_frame.on( 'select', function() {
					...
					$( '#image_' + uploader_id ).attr('src', attachment.url);
					$( '#image_' + uploader_id ).show();
					$( '#reset_' + uploader_id).show();
...
function of_ajax_callback() {
	...
	elseif($save_type == 'uploadmedia'){
			
		update_option($_POST['id'], $_POST['attachment_id']);
	}
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					elseif($type == 'uploadmedia'){
					
						//nothing to do as it is already saved
					}
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "uploadmedia":
			...
			$output .= optionsframework_media_uploader_function($value['id'],$value['std'],null);
		...
		case "heading":
			...
			$jquery_click_hook = preg_replace("[^A-Za-z0-9]", "", str_replace(" ", "-", strtolower($value['name'])));
...
function optionsframework_media_uploader_function($id,$std,$mod){
    ...
    $upload = get_option($id);
	if($upload){
		$upload = wp_get_attachment_url($upload);
	}
	
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button_media" id="'.$id.'">Upload Image</span>';
